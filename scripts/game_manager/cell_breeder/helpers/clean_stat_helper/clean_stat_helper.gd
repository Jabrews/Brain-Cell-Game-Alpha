...
func resolve_clean_stat(stat_array : Array, increase_case_min : float) -> float:
	var stat_high = stat_array[0]
	var stat_low = stat_array[1]
	
	# increase case
	if increase_case_min <= stat_low:
		var result = increase_clean_stat_case.increase_clean_stat_case(
			stat_high,
			stat_low,
			increase_case_min
		)
		...
		if early_game_small_stats:
			...
			# decrease case
			return decrease_clean_stat_case.decrease_clean_stat_case(
				stat_high,
				stat_low,
				increase_case_min
			)
...
func handle_detect_early_stats(stat_high : float, stat_low : float) -> bool:
	...
	# both stats are below 20% of max stat value
