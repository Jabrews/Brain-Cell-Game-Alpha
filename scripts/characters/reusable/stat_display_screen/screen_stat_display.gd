...
func update_screen(designated_brain_cell : BrainCell) : 
	current_cell = designated_brain_cell 
...
func _handle_delete_prisoner_cells():
	if current_cell:
		life_span_label.text = 'lifespan - ' + str(current_cell.life_span)
