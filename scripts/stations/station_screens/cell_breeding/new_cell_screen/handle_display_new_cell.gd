...
func _display_new_cell(
	copy_main_cell_left : BrainCell,
	copy_main_cell_right : BrainCell,
) -> void:
	
	parent_cell_1 = copy_main_cell_left
	parent_cell_2 = copy_main_cell_right
	
	if not parent_cell_1 or not parent_cell_2:
		_reset()
		return
	
	active_cell = _create_new_cell_from_copies(
		copy_main_cell_left,
		copy_main_cell_right
	)
	
	if not active_cell:
		push_error("Could not create new cell preview.")
		_reset()
		return
...
func _create_new_cell_from_copies(
	copy_main_cell_left : BrainCell,
	copy_main_cell_right : BrainCell,
) -> BrainCell:
