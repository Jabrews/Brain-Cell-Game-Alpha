...
@onready var shot_charges_left : Label = $ChargestLeft


func _ready() -> void:
	GLUsableItemBus.connect('toggle_show_defect_pop_up', _handle_toggle_show_defect_pop_up)

func _handle_toggle_show_defect_pop_up(toggle_value : bool, new_selected_cell : BrainCell, new_useable_item_obj : UseableItemObject ) :
		match toggle_value :	
			true : 
				...
				shot_charges_left.text = str(new_useable_item_obj.item_energy)
